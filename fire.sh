...
firewalld_openipport() {
	...
	if [[ -z "${port}" ]]; then
	    ...
	    if [[ -z "${address}" ]]; then
	       ...
	       echo && echo -n -e "请输入协议: " && read protocol
		   if [[ -z "${protocol}" ]]; then
               firewall-cmd --zone=public --source=${address} --add-port=${port}/tcp --permanent
			   firewall-cmd --zone=public --source=${address} --add-port=${port}/udp --permanent
			   firewall-cmd --reload
           else
...
firewalld_closeipport() {
	...
	if [[ -z "${port}" ]]; then
	    ...
	    if [[ -z "${address}" ]]; then
	       ...
	       echo && echo -n -e "请输入协议: " && read protocol
		   if [[ -z "${protocol}" ]]; then
               firewall-cmd --zone=public --source=${address} --remove-port=${port}/tcp --permanent
			   firewall-cmd --zone=public --source=${address} --remove-port=${port}/udp --permanent
			   firewall-cmd --reload
           else
